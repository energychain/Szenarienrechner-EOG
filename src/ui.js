...
function renderManagementSummary(result, first, spread, decision, metrics) {
  ...
  document.getElementById('managementStory').textContent = result.activeMeasures.length
    ? `Bei ${fmtTeur(result.invest)} Investition und ${activeText} liegt die modellierte EOG-Wirkung im Startjahr ${startYear} bei ${fmtTeur(metrics.yearOneRegulatoryEog, 1)} inklusive ${fmtTeur(metrics.yearOneOneOff, 1)} Einmaleffekt nur im Startjahr. Der erste Folgejahreswert ${firstFollowYear} liegt bei ${fmtTeur(metrics.recurringRegulatoryEog, 1)}; spätere Jahreswerte können durch AfA, Wirkungsverzüge, Reinvestitionen oder Rückbau abweichen. Die indikative Portfolio-${metricLabel} beträgt ${irrText}. ${spreadSentence}${result.tariffImpact.available ? ` Für einen Durchschnittshaushalt entspricht der erste Folgejahreswert ${firstFollowYear} rechnerisch etwa ${tariff.value}.` : ''}`
...
function renderReport(result, first, spread, decision, metrics) {
  ...
  const story = result.activeMeasures.length
    ? `Bei ${fmtTeur(result.invest)} Investition und ${activeText} liegt die modellierte EOG-Wirkung im Startjahr ${startYear} bei ${fmtTeur(metrics.yearOneRegulatoryEog, 1)} inklusive ${fmtTeur(metrics.yearOneOneOff, 1)} Einmaleffekt nur im Startjahr. Der erste Folgejahreswert ${firstFollowYear} liegt bei ${fmtTeur(metrics.recurringRegulatoryEog, 1)}; spätere Jahreswerte können abweichen. IRR und Kapitalwert sind indikative Cashflow-Kennzahlen: Portfolio-IRR ${Number.isFinite(result.irr) ? fmtPct(result.irr * 100, 1) : 'nicht berechenbar'} bei einem FK-Zins von ${fmtPct(result.p.financingRate * 100, 1)}.`
